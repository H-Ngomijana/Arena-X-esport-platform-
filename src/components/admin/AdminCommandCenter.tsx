import { Gamepad2, Trophy, Activity, Users, Swords, AlertTriangle, CheckCircle, Wallet } from "lucide-react";
...
const AdminCommandCenter = () => {
  ...
  const metrics = [
    ...
    { label: "Tournaments", value: tournaments.length, icon: Trophy, color: "amber", pulse: false },
    {
      ...
      value: tournaments.filter((t) => t.status === "in_progress" || t.status === "registration_open").length,
      icon: Activity,
      color: "emerald",
      pulse: true,
    },
    ...
    { label: "Paid Entries", value: successfulPayments.length, icon: Wallet, color: "rose", pulse: false },
  ];
    ...
    <div className="space-y-6">
      <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/20 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm text-emerald-300">ALL SYSTEMS OPERATIONAL - ArenaX Control Room v1.0</span>
        </div>
        <span className="font-mono text-xs text-emerald-400/60 hidden sm:block">{new Date().toLocaleString()}</span>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
            ...
            {recentMatches.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                  <span className="font-mono text-[10px] bg-white/5 px-2 py-0.5 rounded text-white/40">R{m.round}</span>
                  <span className="font-mono text-xs text-white/70 flex-1 truncate">{m.team_a.name} vs {m.team_b.name}</span>
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-5">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
            ...
            <span className="font-mono text-[10px] text-white/30">
              Open Disputes: {adminDisputes.filter((d) => d.status === "open").length}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-mono text-sm text-white/70">Recent Paid Registrations</span>
        </div>
        {recentPayments.length === 0 ? (
          <div className="font-mono text-xs text-white/40">No successful MTN payments yet.</div>
        ) : (
          <div className="space-y-2">
            {recentPayments.map((tx) => (
              <div key={tx.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                <span className="font-mono text-[10px] bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded">PAID</span>
                <span className="font-mono text-xs text-white/70 flex-1 truncate">
                  {tx.team_id} {"->"} {tx.tournament_name}
                </span>
                <span className="font-mono text-xs text-white/50">{tx.amount} {tx.currency}</span>
              </div>
            ))}
          </div>
        )}
